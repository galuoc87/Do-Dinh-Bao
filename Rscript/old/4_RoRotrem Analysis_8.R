 ...
 ungroup()
  ...
  mutate(Nu.RoRo.cust = floor(Total.pallet / 48),
    ...
    write.csv(Check.erro.Rodo, file = "C:/Users/Bao/Desktop/errorRodo.csv", row.names = FALSE)
      ...
      group_by(transport.Date,Dest.State) %>%
    ...
    # Final consolidation        
          ...
          mutate(S.Truck.size_State= ifelse( Consolidation.case == 1 & Consol.state <48 ,
            ...
            mutate( Lookup.state.rodo = paste(Fiscal.Unit,str_to_upper(Dest.City..SAP.),"BR012",sep="-"),
                    ...
                    Cost.state = ifelse(S.Truck.code=="0",0,(df.cost.transportation$TARIFA.FRETE[match(Lookup.state.rodo,df.cost.transportation$Look.Up)]*Nu.RoRo.state) +
                                  df.cost.transportation$TARIFA.FRETE[match(Lookup.state.s,df.cost.transportation$Look.Up)]
                                   )
                                   ) 
          ...
          write.csv(RoRoTrem.Analysis_state, file = "C:/Users/Bao/Desktop/consolidation at state.csv", row.names = FALSE)
            ...
            mutate( Lookup.final.rodo = paste(Fiscal.Unit,str_to_upper(Dest.City..SAP.),"BR012",sep="-"),
                    ...
                    Cost.final = ifelse(S.Truck.code.f=="0",0,(df.cost.transportation$TARIFA.FRETE[match(Lookup.final.rodo,df.cost.transportation$Look.Up)]*Nu.RoRo.final) +
                                          df.cost.transportation$TARIFA.FRETE[match(Lookup.final.s,df.cost.transportation$Look.Up)]
                    )
            )     
            ...
            Sumtab_RoDotrem_3 <- Sample.T1.1.Delivery %>%
            ...
            write.csv(Sumtab_RoDotrem_3, file = "C:/Users/Bao/Desktop/Customer List.csv", row.names = FALSE)
