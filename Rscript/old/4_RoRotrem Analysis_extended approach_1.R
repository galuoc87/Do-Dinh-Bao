...
RoRoTrem_extended <-RoRoTrem_extended %>%
  rowwise() %>%
...
RoRoTrem_extended <-RoRoTrem_extended %>%
  ...
  mutate(S.Truck.size.cust =          (Re.pal.cust>0)*(Re.pal.cust<=1.5)*1
                                     ...
                                     +(Re.pal.cust>12.5)*(Re.pal.cust<=28.7)*28
                                     #(Re.pal.state>0)*(Re.pal.state<=28)*28
                                     +(Re.pal.cust>28.7)*48) # Assign the suitatble truck for remaining
 ...
 # [Step2]-create indication at the Microrregiao level:
  ...
  RoRoTrem_extended <-RoRoTrem_extended %>%
    ...
    mutate(S.Truck.size.micro = ifelse( Re.pal.micro ==0 ,0,
                                       ...
                                       +(Re.pal.micro>12.5)*(Re.pal.micro<=28.7)*28
                                       #(Re.pal.state>0)*(Re.pal.state<=28)*28
                                       +(Re.pal.micro>28.7)*48 # Assign the suitatble truck for remaining
                                       )) 
  ...
  sum(RoRoTrem_conservative$Nu.RoRo.cust)+sum(RoRoTrem_conservative$Nu.RoRo.micro)+sum(RoRoTrem_conservative$Shipment.micro.rodo)
  sum(RoRoTrem_conservative$shipment.micro)-sum(RoRoTrem_conservative$Shipment.micro.rodo)
  sum(RoRoTrem_conservative$current.frei.cost)
  ...
  # [Step 2]check the error in the rate for micro
    ...
    RoRoTrem_extended_state <- RoRoTrem_extended %>%
    ...
    # Summary table to calculate 
    
    Sumtab_RoDotrem_3 <- RoRoTrem_extended_state %>%
      
      group_by(Dest.State) %>%
      summarise(Current.cost= mean(Total.current),
                Cust.cost = mean(Total.cost.cust),
                micro.cost = mean(Total.cost.micro),
                state.cost = sum(Cost.state) + sum(Other.cost.state),
                Cost.diff = Cust.cost + micro.cost + state.cost - Current.cost,
                Nu.Roro.Shipment= mean(Total.Rodo.micro) + sum(Nu.RoRo.state) + sum(Shipment.state.rodo),
                Other.shipments = sum(shipment.state) - sum(Shipment.state.rodo)
      )
