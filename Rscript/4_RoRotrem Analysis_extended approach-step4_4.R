...
Sumtab_RoDotrem_4 <- RoRoTrem_conservative_s2 %>% # because after fillter we choose step 2 to do report
  group_by(Dest.State,Dest.Code) %>%
...
write.csv(Sumtab_RoDotrem_4, file = "Rodotrem.customer2.csv", row.names = FALSE)

write.csv(RoRoTrem_conservative, file = "Rodotrem.analysis.client.Araras2.csv", row.names = FALSE)


write.csv(RoRoTrem_conservative, file = "Rodotrem.analysis.client.Garanhus.csv", row.names = FALSE)
...
write.csv(RoRoTrem_conservative_s2, file = "Rodotrem.analysis.city.Araras.csv", row.names = FALSE)
...
Sumtab_RoDotrem_5 <-RoRoTrem_conservative_s3 %>%
  group_by(Dest.State,First.geography) %>%
  summarise(Current.cost = sum(current.frei.cost),
            Current.shipment = sum (Nu.shipment),
            Total.weight.tons = sum (Total.weight)/1000,
...
write.csv(RoRoTrem_conservative_s2, file = "Rodotrem.analysis.city.Garaanhus.csv", row.names = FALSE)

sum(Sumtab_RoDotrem_5$Cost.diff)
...
RoRoTrem_conservative_s3 <- RoRoTrem_conservative_s3 %>%
  arrange(transport.Date,Dest.State,Distance.gps) %>%
...
RoRoTrem_conservative_s3_2<-RoRoTrem_conservative_s3 %>%
  filter(Cost.diff.state <10000)


# [Step 3a] Summary table to calculate 

Sumtab_RoDotrem_7 <- RoRoTrem_conservative_s3_2 %>%
...
sum(Sumtab_RoDotrem_7$Cost.diff)
...
write.csv(RoRoTrem_conservative_s3 , file = "Rodotrem.analysis.state.Araras.csv", row.names = FALSE)

write.csv(RoRoTrem_conservative_s3 , file = "Rodotrem.analysis.state.Garanhus.csv", row.names = FALSE)
...
RoRoTrem_conservative_s3_2 <- RoRoTrem_conservative_s3_2 %>%
  arrange(transport.Date,Replace.Dest.State,Distance.gps) %>%
  group_by(transport.Date,Replace.Dest.State) %>%# Change the Des.state or replace des.state
  filter(Total.pallet>0) %>% # remove the record with 0 value
  mutate(Rank.Date=row_number(Distance.gps)) %>%
  mutate(Max.Date=max(Rank.Date)) %>%
  mutate(Cumpallet=cumsum(Total.pallet)) %>% # add cum pallet
...
RoRoTrem_conservative_s4 <- RoRoTrem_conservative_s3_2 %>%
            group_by(transport.Date,Replace.Dest.State) %>% 
        ...
        #  mutate(S.Truck.size_State= ifelse( Consolidation.case == 1 & Consol.state <50 ,
         #          (Re.pal.state>0)*(Re.pal.state<=1.5)*1
          ...
          RoRoTrem_conservative_s4 <-  RoRoTrem_conservative_s4 %>%
         ...
         # Create the summary table for state
           ...
           # Sumtab_RoDotrem_3 <- RoRoTrem.Analysis_state %>%
            ...
            #Sumtab_RoDotrem_5 <- Sample.T1.1.Delivery %>%
              ...
              Sumtab_RoDotrem_8 <-  RoRoTrem_conservative_s4 %>%
              ...
              sum(Sumtab_RoDotrem_8$Cost.diff)
              ...
              setwd("C:/Users/Bao/Desktop/") #company
              write.csv(Sumtab_RoDotrem_8, file = "Rodotrem.across.state.csv", row.names = FALSE)

              
              
                 write.csv(RoRoTrem_conservative_s4, file = "roro.step4.csv", row.names = FALSE)
                 write.csv(RoRoTrem_conservative_s4, file = "Rodotrem.across.state.Garanhus.csv", row.names = FALSE)
