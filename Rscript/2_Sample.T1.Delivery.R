...
Sample.T1.delivery <-df.trans.forT1 %>%
  ...
  filter(Occurence==1)%>%
mutate(Cbind_Custype_Shiptype = str_c(T1.T2,Shipment.type,sep="-")) 
...
filter(
  ...
  Cbind_Custype_Shiptype %in% c("T1-VENDA","T1-TNF","T1-TRANSFERENCIA"), 
...
# Try to fix the string of date
  ...
  arrange(Dates,`Dest.City (SAP)`) %>%
  
# Choose the suitable Truck for the weight.
  mutate(Suit.Truck.Size=
           (Gross.weight.Kg>0)*(Gross.weight.Kg<=1300)*1300
         +(Gross.weight.Kg>1300)*(Gross.weight.Kg<=3000)*3000
         +(Gross.weight.Kg>3000)*(Gross.weight.Kg<=6000)*6000
         +(Gross.weight.Kg>6000)*(Gross.weight.Kg<=10800)*10800
         +(Gross.weight.Kg>10800)*(Gross.weight.Kg<=27000)*27000
         +(Gross.weight.Kg>27000)*42000) %>%
 ...
 # Create the lookup columns in transport data following the format
  ...
  mutate(Lookup.trans= paste(Fiscal.Unit,str_to_upper(`Dest.City (SAP)`),Shipment.type,Vehicle.SAP.code,Carrier.SAP.code,sep="-")) %>%
  ...
  # Calculate the Current transportation Cost.
    ...
    # Create the lookup columns in transport data following the format
    mutate(Suit.Lookup.trans= paste(Fiscal.Unit,str_to_upper(`Dest.City (SAP)`),Shipment.type,Suit.Truckcode,Carrier.SAP.code,sep="-")) %>%
